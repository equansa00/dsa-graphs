...
describe("DFS", function() {
  it("returns an array of the nodes searched using DFS", function() {
    ...
    let result = graph.depthFirstSearch(S);
    let possibleResults = [
      ["S", "U", "V", "W", "T", "R", "Y", "X", "Q", "P"],
      ["S", "P", "X", "Y", "W", "V", "U", "R", "T", "Q"]
    ];

    // Check if the result matches any of the possible valid results
    expect(possibleResults.some(order => JSON.stringify(order) === JSON.stringify(result))).toBe(true);
  });
});
